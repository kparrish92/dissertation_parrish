...
knitr::include_graphics(here::here("includes", "figures", "conditions.png"))
```
...
In order to examine perception of vowels, a phoneme categorization task will be carried out in both German and French in separate sessions, carried out on different days.
During this task, participants will hear tokens produced by L1 German or French speakers, in which a particular vowel sound is embedded in a particular consonant frame, which will vary by condition. 
After each sound, the participants will see both English and French carrier words on the screen.
They will be instructed to select the word that contains a vowel sound that most closely matches the stimulus that they heard per trial. 
Then, participants will then rate their selection for goodness of fit on a slider scale by clicking. 
The scale is continuous and ranges from 1-7, but allows for any point on the scale to be clicked, producing a rating that is not necessarily discrete.  
In the VOT bias conditions, the vowel will be uttered in combination with either an aspirated (German) or unaspirated (French) syllable-initial stop. 
In the conditions in which VOT bias is not present, a fricative-vowel-fricative frame will be used. 
Participants will categorize 5 repetitions of each condition, resulting in 40 total categorizations per language. 

In terms of discrimination, the present dissertation will focus on English vowel contrasts and VOT stop contrasts in two separate continua embedded in L3 words. 
